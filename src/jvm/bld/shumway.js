...
if (!jsGlobal.performance.now) {
  jsGlobal.performance.now =
    typeof dateNow !== "undefined" ? dateNow : Date.now;
}
...
function warn(message) {
  ...
  if (inBrowser) {
    ...
    jsGlobal.print(
      Shumway.IndentingWriter.RED + message + Shumway.IndentingWriter.ENDC
    );
  }
}
var Shumway;
(function (Shumway) {
  ...
  (function (Debug) {
    ...
    function untested(message) {
      Debug.warning(
        "Congratulations, you've found a code path for which we haven't found a test case. Please submit the test case: " +
          message
      );
    }
  ...
  var IndentingWriter = (function () {
    ...
    IndentingWriter.logLevel = LogLevel.All;
    IndentingWriter.stdout = inBrowser ? console.info.bind(console) : print;
    IndentingWriter.stdoutNoNewline = inBrowser
  ...
  var ColorStyle = (function () {
    ...
    ColorStyle.randomStyle = function () {
      ...
      return ColorStyle._randomStyleCache[
        ColorStyle._nextStyle++ % ColorStyle._randomStyleCache.length
      ];
    };
    ColorStyle.contrastStyle = function (rgb) {
      ...
      var yiq =
        ((c >> 16) * 299 + ((c >> 8) & 0xff) * 587 + (c & 0xff) * 114) / 1000;
  ...
  (function (StringUtilities) {
    ...
    function variableLengthEncodeInt32(n) {
      ...
      release ||
        assert(
          StringUtilities.variableLengthDecodeInt32(s) === n,
          n + " : " + s + " - " + l + " bits: " + bitCount
        );
      return s;
    }
    ...
    function trimMiddle(s, maxLength) {
      ...
      return (
        s.substr(0, leftHalf) +
        "\u2026" +
        s.substr(s.length - rightHalf, rightHalf)
      );
    }
...
(function (Shumway) {
  ...
  (function (Tools) {
    ...
    (function (Profiler) {
      ...
      var TimelineFrame = (function () {
        ...
        TimelineFrame.prototype.calculateStatistics = function () {
          ...
          function visit(frame) {
            if (frame.kind) {
              var s =
                statistics[frame.kind.id] ||
                (statistics[frame.kind.id] = new TimelineFrameStatistics(
                  frame.kind
                ));
              s.count++;
        ...
        TimelineFrame.prototype.traceStatistics = function (
          writer,
          minTime,
          format
        ) {
        ...
        TimelineFrame.prototype.trace = function (writer, minTime) {
          if (minTime === void 0) {
            minTime = 0.0001;
          }
          ...
          var s =
            (this.kind ? this.kind.name + ": " : "Profile: ") + time.toFixed(3);
      ...
      var TimelineBufferSnapshotSet = (function (_super) {
        __extends(TimelineBufferSnapshotSet, _super);
        function TimelineBufferSnapshotSet(snapshots) {
          _super.call(this, null);
          this.snapshots = snapshots;
        }
        TimelineBufferSnapshotSet.prototype.calculateStatistics = function () {
          var snapshots = this.snapshots;
          var aggregateStats = {};
          for (var i = 0; i < snapshots.length; i++) {
            var snapshot = snapshots[i];
            var statistics =
              snapshot.statistics || snapshot.calculateStatistics();
            for (var j = 0; j < statistics.length; j++) {
              var stat = statistics[j];
              if (stat) {
                var name = stat.kind ? stat.kind.name : "?";
                var aggregateStat = (aggregateStats[name] =
                  aggregateStats[name] ||
                  new TimelineFrameStatistics(stat.kind));
          ...
          return (this.statistics = Object.keys(aggregateStats).map(function (
            key
          ) {
            return aggregateStats[key];
          }));
        };
        return TimelineBufferSnapshotSet;
      })(TimelineBufferSnapshot);
      Profiler.TimelineBufferSnapshotSet = TimelineBufferSnapshotSet;
    })((Profiler = Tools.Profiler || (Tools.Profiler = {})));
  })((Tools = Shumway.Tools || (Shumway.Tools = {})));
})(Shumway || (Shumway = {}));
/**
    ...
    (function (Profiler) {
      ...
      var TimelineBuffer = (function () {
        function TimelineBuffer(name, startTime) {
          ...
          this.name = name || "";
          this._startTime = Shumway.isNullOrUndefined(startTime)
            ? jsGlobal.START_TIME
            : startTime;
        }
        ...
        TimelineBuffer.prototype._initialize = function () {
          ...
          this._marks =
            TimelineBuffer.preAllocInt32ArrayBuffers.pop() ||
            new Shumway.CircularBuffer(Int32Array, TimelineBuffer.BUFFER_SIZE);
          this._times =
            TimelineBuffer.preAllocFloat64ArrayBuffers.pop() ||
            new Shumway.CircularBuffer(
              Float64Array,
              TimelineBuffer.BUFFER_SIZE
            );
        };
        ...
        TimelineBuffer.prototype._getMark = function (type, kindId, data) {
          ...
          if (
            !Shumway.isNullOrUndefined(data) &&
            kindId !== TimelineBuffer.MAX_KINDID
          ) {
        ...
        TimelineBuffer.prototype.enter = function (name, data, time) {
          time =
            (Shumway.isNullOrUndefined(time) ? performance.now() : time) -
            this._startTime;
          if (!this._marks) {
            this._initialize();
          }
        ...
        TimelineBuffer.prototype.createSnapshot = function (count) {
          ...
          if (snapshot.children && snapshot.children.length) {
            ...
            snapshot.endTime =
              snapshot.children[snapshot.children.length - 1].endTime;
          }
          return snapshot;
        };
        ...
        TimelineBuffer.FromFirefoxProfile = function (profile, name) {
          ...
          for (var i = 0; i < samples.length; i++) {
            ...
            while (
              j < minStackLen &&
              stack[j].location === currentStack[j].location
            ) {
              j++;
            }
        ...
        TimelineBuffer.BUFFER_SIZE = 20;
        TimelineBuffer.preAllocInt32ArrayBuffers = [];
        TimelineBuffer.preAllocFloat64ArrayBuffers = [];
      ...
      for (var i = 0; i < 64; i++) {
        TimelineBuffer.preAllocInt32ArrayBuffers.push(
          new Shumway.CircularBuffer(Int32Array, TimelineBuffer.BUFFER_SIZE)
        );
        TimelineBuffer.preAllocFloat64ArrayBuffers.push(
    ...
    (function (Profiler) {
      var Controller = (function () {
        ...
        Controller.prototype._createViews = function () {
          if (this._activeProfile) {
            var self = this;
            this._overviewHeader = new Profiler.FlameChartHeader(
              this,
              0 /* OVERVIEW */
            );
            this._overview = new Profiler.FlameChartOverview(
              this,
              0 /* OVERLAY */
            );
            this._activeProfile.forEachSnapshot(function (snapshot, index) {
              self._headers.push(
                new Profiler.FlameChartHeader(
                  self,
                  1 /* CHART */,
                  snapshot.name
                )
        ...
        Controller.prototype.createTooltipContent = function (chart, frame) {
          ...
          var selfPercent =
            Math.round((frame.selfTime * 100 * 100) / frame.totalTime) / 100;
          ...
          elSelfTime.textContent =
            "Self: " + selfTime + " ms (" + selfPercent + "%)";
          ...
          if (statistics) {
            ...
            var allTotalTime =
              Math.round(statistics.totalTime * 100000) / 100000;
    ...
    (function (Profiler) {
      ...
      var MouseController = (function () {
        function MouseController(target, eventTarget) {
          ...
          eventTarget.addEventListener(
            "mouseover",
            this._boundOnMouseOver,
            false
          );
          eventTarget.addEventListener(
            "mouseout",
            this._boundOnMouseOut,
            false
          );
          eventTarget.addEventListener(
        ...
        MouseController.prototype._startHoverCheck = function (event) {
          this._hoverInfo = {
            ...
            timeoutHandle: setTimeout(
              this._onMouseMoveIdleHandler.bind(this),
              MouseController.HOVER_TIMEOUT
            ),
        ...
        MouseController.prototype._getTargetMousePos = function (
          event,
          target
        ) {
    ...
    (function (Profiler) {
      var FlameChartBase = (function () {
        function FlameChartBase(controller) {
          ...
          this._mouseController = new Profiler.MouseController(
            this,
            this._canvas
          );
        ...
        FlameChartBase.prototype._windowEqRange = function () {
          return (
            this._almostEq(this._windowStart, this._rangeStart) &&
            this._almostEq(this._windowEnd, this._rangeEnd)
          );
        };
        ...
        FlameChartBase.prototype.onMouseWheel = function (x, y, delta) {
          ...
          var maxDelta = Math.max(
            (FlameChartBase.MIN_WINDOW_LEN - windowLen) / windowLen,
            delta
          );
        ...
        FlameChartBase.prototype.onDrag = function (
          startX,
          startY,
          currentX,
          currentY,
        ...
        FlameChartBase.prototype.onDragEnd = function (
          startX,
          startY,
          currentX,
          currentY,
    ...
    (function (Profiler) {
      ...
      var FlameChart = (function (_super) {
        ...
        FlameChart.prototype.setSize = function (width, height) {
          _super.prototype.setSize.call(
            this,
            width,
            height || this._initialized ? this._maxDepth * 12.5 : 100
          );
        };
        ...
        FlameChart.prototype._drawFrame = function (frame, depth) {
          ...
          if (width <= this._minFrameWidthInPixels) {
            ...
            context.fillRect(
              left,
              depth * (12 + frameHPadding),
              this._minFrameWidthInPixels,
              12 + (frame.maxDepth - frame.depth) * 12.5
            );
            return false;
          }
          if (left < 0) {
          ...
          if (width > 12) {
            ...
            if (label && label.length) {
              var labelHPadding = 2;
              label = this._prepareText(
                context,
                label,
                adjustedWidth - labelHPadding * 2
              );
              if (label.length) {
        ...
        FlameChart.prototype.onDrag = function (
          startX,
          startY,
          currentX,
          currentY,
        ...
        FlameChart.prototype.onDragEnd = function (
          startX,
          startY,
          currentX,
          currentY,
    ...
    (function (Profiler) {
      ...
      var FlameChartOverview = (function (_super) {
        ...
        FlameChartOverview.prototype._drawSelection = function () {
          ...
          var height = this._height;
          var ratio = window.devicePixelRatio;
          var left = this._selection
            ? this._selection.left
            : this._toPixels(this._windowStart);
          var right = this._selection
            ? this._selection.right
            : this._toPixels(this._windowEnd);
          ...
          context.stroke();
          // Draw info labels
          var start = this._selection
            ? this._toTime(this._selection.left)
            : this._windowStart;
          var end = this._selection
            ? this._toTime(this._selection.right)
            : this._windowEnd;
        ...
        FlameChartOverview.prototype._getDragTargetUnderCursor = function (
          x,
          y
        ) {
        ...
        FlameChartOverview.prototype.onMouseMove = function (x, y) {
          ...
          if (dragTarget !== 0 /* NONE */ && !this._selection) {
            cursor =
              dragTarget === 1 /* WINDOW */
                ? Profiler.MouseCursor.GRAB
                : Profiler.MouseCursor.EW_RESIZE;
          }
        ...
        FlameChartOverview.prototype.onDrag = function (
          startX,
          startY,
          currentX,
          currentY,
          ...
          if (this._selection) {
            this._selection = {
              left: startX,
              right: clamp(currentX, 0, this._width - 1),
            };
            this.draw();
          } else {
            var dragInfo = this._dragInfo;
            if (dragInfo.target === 4 /* HANDLE_BOTH */) {
              if (deltaX !== 0) {
                dragInfo.target =
                  deltaX < 0 ? 2 /* HANDLE_LEFT */ : 3 /* HANDLE_RIGHT */;
              } else {
                return;
              }
            }
            ...
            switch (dragInfo.target) {
              ...
              case 2 /* HANDLE_LEFT */:
                windowStart = clamp(
                  dragInfo.windowStartInitial + delta,
                  this._rangeStart,
                  windowEnd - Profiler.FlameChartBase.MIN_WINDOW_LEN
                );
                break;
              case 3 /* HANDLE_RIGHT */:
                windowEnd = clamp(
                  dragInfo.windowEndInitial + delta,
                  windowStart + Profiler.FlameChartBase.MIN_WINDOW_LEN,
                  this._rangeEnd
                );
                break;
              default:
        ...
        FlameChartOverview.prototype.onDragEnd = function (
          startX,
          startY,
          currentX,
          currentY,
          ...
          if (this._selection) {
            ...
            this._controller.setWindow(
              this._toTime(startX),
              this._toTime(currentX)
            );
          }
          this._dragInfo = null;
  ...
  (function (Tools) {
    ...
    (function (Profiler) {
      ...
      var FlameChartHeader = (function (_super) {
        ...
        function FlameChartHeader(controller, type, name) {
          this._type = type;
        ...
        FlameChartHeader.prototype.destroy = function () {
          this._controller.container.removeChild(this._name);
          _super.prototype.destroy.call(this);
        };
        FlameChartHeader.prototype._drawLabels = function (
          rangeStart,
          rangeEnd
        ) {
          var context = this._context;
        ...
        FlameChartHeader.prototype._calculateTickInterval = function (
          rangeStart,
          rangeEnd
        ) {
          ...
          var magnitude = Math.pow(
            10,
            Math.floor(Math.log(minimum) / Math.LN10)
          );
        ...
        FlameChartHeader.prototype._drawDragHandle = function (pos) {
          ...
          this._drawRoundedRect(
            context,
            pos - Profiler.FlameChartBase.DRAGHANDLE_WIDTH / 2,
            1,
            Profiler.FlameChartBase.DRAGHANDLE_WIDTH,
            this._height - 2,
            2,
            true
          );
        };
        FlameChartHeader.prototype._drawRoundedRect = function (
          context,
          x,
          y,
          width,
        ...
        FlameChartHeader.prototype.onDrag = function (
          startX,
          startY,
          currentX,
          currentY,
        ...
        FlameChartHeader.prototype.onDragEnd = function (
          startX,
          startY,
          currentX,
          currentY,
    ...
    (function (Profiler) {
      ...
      (function (TraceLogger_1) {
        var TraceLoggerProgressInfo = (function () {
          function TraceLoggerProgressInfo(
            pageLoaded,
            threadsTotal,
            threadsLoaded,
            threadFilesTotal,
        ...
        var TraceLogger = (function () {
          ...
          TraceLogger.prototype._loadData = function (
            urls,
            callback,
            progress
          ) {
        ...
        var Thread = (function () {
          ...
          Thread.prototype._walkTree = function (id) {
            ...
            do {
              ...
              var start =
                data.getUint32(index + Offsets.START_HI) * 4294967295 +
                data.getUint32(index + Offsets.START_LO);
              var stop =
                data.getUint32(index + Offsets.STOP_HI) * 4294967295 +
                data.getUint32(index + Offsets.STOP_LO);
          ...
          function onKeyDown(event) {
            ...
            switch (event.keyCode) {
              ...
              case KEY_C:
                if (event.metaKey) {
                  ...
                  if (this.selection) {
                    for (
                      var i = this.selection.start;
                      i <= this.selection.end;
                      i++
                    ) {
            ...
            if (delta && event.shiftKey) {
              if (!this.selection) {
                if (delta > 0) {
                  this.selection = {
                    start: this.lineIndex - delta,
                    end: this.lineIndex,
                  };
                } else if (delta < 0) {
                  this.selection = {
                    start: this.lineIndex,
                    end: this.lineIndex - delta,
                  };
                }
              } else {
        ...
        Terminal.prototype.paint = function () {
          ...
          var lineTimeMargin =
            ...
            (this.showLineNumbers
              ? (String(this.buffer.length).length + 2) * charSize
              : 0);
          var lineRepeatMargin =
            lineTimeMargin + (this.showLineTime ? charSize * 8 : 2 * charSize);
          var lineMargin = lineRepeatMargin + charSize * 5;
          this.context.font =
            this.fontSize +
            'px Consolas, "Liberation Mono", Courier, monospace';
          ...
          for (var i = 0; i < lineCount; i++) {
            ...
            var lineTimeDelta =
              lineIndex > 1
                ? this.buffer.getTime(lineIndex) - this.buffer.getTime(0)
                : 0;
            this.context.fillStyle =
              lineIndex % 2 ? this.lineColor : this.alternateLineColor;
            ...
            if (
              this.selection &&
              lineIndex >= this.selection.start &&
              lineIndex <= this.selection.end
            ) {
            ...
            if (this.showLineTime) {
              this.context.fillText(
                lineTimeDelta.toFixed(1).padLeft(" ", 6),
                lineTimeMargin,
                marginTop
              );
            }
    ...
    (function (Mini) {
      var FPS = (function () {
        ...
        FPS.prototype.tickAndRender = function (idle, renderTime) {
          ...
          var weightedTime =
            elapsedTime * (1 - weightRatio) +
            this._lastWeightedTime * weightRatio;
          var context = this._context;
          var w = 2 * this._ratio;
          var wPadding = 1;
          var fontSize = 8;
          ...
          if (index % 16 === 0) {
            ...
            if (webkitPerformance.memory) {
              s +=
                " " +
                (webkitPerformance.memory.usedJSHeapSize / 1024 / 1024).toFixed(
                  2
                );
            }
            context.fillText(
              s,
              2 * this._ratio,
              (this._containerHeight / 2) * this._ratio
            );
          }
          this._lastTime = performance.now();
